    ...
    async with engine.begin() as conn:
        ...
        if c_id and d_id:
            await conn.execute(text("""
                ...
                VALUES (gen_random_uuid(), :c_id, :d_id, 'whatsapp', 'auto_send', 0.9, 0.15, 30, 'pending')
            """), {"c_id": c_id, "d_id": d_id})
            print("Inserted a test reminder (NO DISCOUNT) into pending_reminders")
    await engine.dispose()

if __name__ == "__main__":
    asyncio.run(seed_no_discount())
