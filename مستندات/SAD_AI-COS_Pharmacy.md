...
## 3. تنظيم الـ Domains (بدون تخصيص أفراد — للتوثيق المعماري فقط)

هذا القسم يوثّق **حدود المسؤولية بين الـ Domains** كمرجع معماري، بغض النظر عن تفاصيل من يعمل على كل جزء عمليًا:

| Domain (من AI-COS الأصلي) | التقنيات الأساسية المستخدمة |
|---|---|
| Identity & Access, Multi-Tenant | Supabase Auth, RLS |
| Store, Product Catalog, Orders, Payments | FastAPI, SQLAlchemy |
| Behavioral Tracking, Notifications | Session/Event Tables, n8n |
| Feature Store, Data Engineering | Pandas, DuckDB, Postgres |
| Machine Learning Models, Continuous Learning | Scikit-learn, XGBoost, MLflow, SHAP |
| AI Agents, Knowledge/RAG, Executive Copilot | LangGraph, MCP, LlamaIndex, Gemma |
| Workflow Automation, Analytics, Monitoring | n8n, Prometheus, Grafana |
| Frontend (متجر + Dashboard) | Next.js, React, ECharts |

> المبدأ الثابت: **كل Domain حدوده واضحة ومستقلة في الكود** (مجلد منفصل تحت `domains/`)، بحيث يمكن العمل على أي جزء بمعزل عن الباقي دون تعارض، بغض النظر عن عدد الأشخاص الفعلي المنفّذ.

---

## 4. عقد التكامل بين النموذج التنبؤي ومحرك القرار (Data/ML ↔ AI Decision Engine Contract)

نقطة تكامل حرجة يجب توثيقها **قبل** كتابة أي كود، لتفادي أي تعارض لاحق بين مخرجات النموذج ومدخلات محرك القرار:

```json
{
  "customer_id": "uuid",
  "drug_id": "uuid",
  "prediction_type": "refill_reminder | churn_risk",
  "predicted_value": "2026-08-15",
  "confidence_score": 0.87,
  "model_version": "xgboost_v2",
  "explanation": {
    "top_features": [
      {"feature": "avg_cycle_days", "impact": 0.42},
      {"feature": "browsing_frequency", "impact": 0.21}
    ]
  },
...
- أي نموذج جديد (Baseline أو متقدم) **يجب أن يُخرج نفس الصيغة بالضبط**، بغض النظر عن الخوارزمية الداخلية.
- محرك القرار (AI Decision Engine) لا يعرف تفاصيل النموذج الداخلية إطلاقًا، بل يتعامل فقط مع هذا الـ Contract الموحّد — وهذا يسمح باستبدال أو تحسين النموذج دون تعديل منطق القرار.

---

## 5. استراتيجية Git Branching

| الفرع | الغرض |
|---|---|
| `main` | نسخة مستقرة فقط، جاهزة للعرض في أي وقت |
| `develop` | خط الدمج الرئيسي لكل التطوير الجاري |
| `feature/<domain-name>` | فرع مستقل لكل Domain (مثال: `feature/ai-decision-engine`) يُدمَج في `develop` عبر Pull Request فقط |

**قاعدة أساسية:** لا يُدمَج أي فرع في `main` مباشرة؛ يمر دائمًا عبر `develop` بعد مراجعة (Code Review)، حتى لو كان التنفيذ الفعلي يتم بشكل مركزي — هذا يحافظ على وجود نسخة "آمنة دائمًا" جاهزة للعرض في أي لحظة دون مفاجآت.

---

## 6. إدارة الـ Secrets والمتغيرات البيئية

| الممارسة | التفصيل |
|---|---|
| `.env.example` | ملف موحّد في الجذر يحتوي أسماء كل المتغيرات المطلوبة (`SUPABASE_URL`, `SUPABASE_KEY`, `OLLAMA_HOST`, `CLOUD_LLM_API_KEY`...) بدون القيم الفعلية |
| `.env` | الملف الحقيقي بالقيم — **لا يُرفَع على Git إطلاقًا** (`.gitignore` من اليوم الأول) |
| قاعدة صريحة | لا تُكتَب أي مفاتيح API حقيقية داخل الكود، أو تُشارَك في أي أداة محادثة (بما فيها Claude) — تُدار فقط عبر ملف `.env` المحلي أو Secrets Manager عند النشر السحابي |

---

## 7. محتوى الـ Shared Kernel

`shared_kernel/` ليس مجرد اسم، بل يحتوي فعليًا على العناصر المشتركة بين كل الـ Domains لمنع تكرار الكود:

| العنصر | الوصف |
|---|---|
| `schemas/tenant_context.py` | Pydantic Schema موحّد لسياق الـ Tenant (يُستخدم في كل طلب) |
| `schemas/customer_base.py` | الحقول الأساسية للعميل (المشتركة بين كل الـ Domains التي تحتاج بيانات العميل) |
| `auth/dependencies.py` | Dependency موحّدة للتحقق من هوية المستخدم وصلاحياته (تُستخدم في كل Endpoint محمي) |
| `audit/logger.py` | دالة واحدة لتسجيل أي حدث مهم في `AuditLog`، تُستدعى من أي Domain دون إعادة كتابة منطق التسجيل |

---

## 8. طبقة تنسيق الـ AI Agents (LangGraph + MCP)
...
## 9. استراتيجية النشر (Deployment Strategy)
